...
const fragmentShader = `
  ...
  void main() {
    ...
    // Map luminance to violet palette
    float lum = dot(cppn.rgb, vec3(0.299, 0.587, 0.114));

    vec3 black        = vec3(0.0);
    vec3 violet900    = vec3(0.298, 0.114, 0.584); // #4c1d95
    vec3 violet500    = vec3(0.545, 0.361, 0.965); // #8b5cf6

    vec3 color = mix(black, violet900, clamp(lum * 2.2, 0.0, 1.0));
    color += violet500 * pow(clamp(lum, 0.0, 1.0), 1.8) * 1.4;
...
export function NeuralBackground() {
  ...
  useGSAP(
    () => {
      ...
      gsap.set(canvasRef.current, { scale: 1.1, autoAlpha: 0.7 });
  ...
  // Skip WebGL rendering on mobile or with prefers-reduced-motion
